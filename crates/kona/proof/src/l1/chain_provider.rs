...
#[derive(Debug, Clone)]
pub struct CeloOracleL1ChainProvider<T: CommsClient> {
    /// The L1 head hash.
    pub l1_head: B256,
...
impl<T: CommsClient> CeloOracleL1ChainProvider<T> {
...
#[async_trait]
impl<T: CommsClient + Sync + Send> CeloChainProvider for CeloOracleL1ChainProvider<T> {
    ...
    async fn block_info_and_transactions_by_hash(
        ...
        let transactions = trie_walker
            ...
            .map(|(_, rlp)| {
                ...
                let rlp = CeloTxEnvelope::decode_2718(&mut rlp.as_ref())?;
                Ok(rlp)
            })
            .collect::<Result<Vec<_>, _>>()
            .map_err(OracleProviderError::Rlp)?;
...
impl<T: CommsClient> TrieProvider for CeloOracleL1ChainProvider<T> {
    type Error = OracleProviderError;
