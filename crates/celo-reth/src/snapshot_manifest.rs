...
use reth_db::{mdbx::DatabaseArguments, open_db, tables, Database};
...
fn infer_blocks_per_file(source_datadir: &Path) -> Result<u64> {
    ...
    inferred.ok_or_else(|| {
        eyre::eyre!(
            "Could not infer --blocks-per-file from header static files; pass it manually"
        )
    })
}
...
fn parse_headers_range(file_name: &str) -> Option<(u64, u64)> {
    ...
    let end_digits: String =
        end_with_suffix.chars().take_while(|ch| ch.is_ascii_digit()).collect();
