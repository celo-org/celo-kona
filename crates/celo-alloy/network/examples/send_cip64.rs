...
//!
//! The reference recipe for external Rust clients: with the `Celo` network and its
//! recommended fillers, setting `fee_currency` is all a caller needs. Nonce, chain id, gas
//! limit (including the CIP-64 intrinsic surcharge) and the fee-currency-denominated fees
//! are filled in, and the wallet signs a type-`0x7b` transaction.
//!
//! Environment:
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Without a timeout, a tx the node never includes would wait here forever.
    let receipt = pending.with_timeout(Some(Duration::from_secs(60))).get_receipt().await?;
    ...
    // Correct filling caps at `2·baseFee + tip` in fee-currency units; native suggestions
    // would land near the base fee itself, off by the currency's exchange rate. Requiring
    // 1.5x separates the two and still tolerates a base fee that rose before inclusion.
