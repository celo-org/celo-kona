...
impl<T: CommsClient + Send + Sync> CeloBatchValidationProvider for OracleL2ChainProvider<T> {
    ...
    async fn block_by_number(&mut self, number: u64) -> Result<CeloBlock, Self::Error> {
        ...
        info!("Celo::block_by_number 2 header={:?}", header);
        let header_hash = header.hash_slow();
        info!("Celo::block_by_number 3 header_hash={:?}", header_hash);
        ...
        info!("Celo::block_by_number 7 optimism_block={:?}", optimism_block);
        Ok(optimism_block)
    }
}
