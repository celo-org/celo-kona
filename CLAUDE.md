...
- **`rpc.rs`** — `CeloRpcTypes`, `CeloReceiptConverter`, `CeloEthApiBuilder` (Celo equivalents of op-reth's `Optimism`/`OpReceiptConverter`/`OpEthApiBuilder`). `eth_gasPrice`/`eth_maxPriorityFeePerGas` take an optional `feeCurrency` and scale by its rate. CIP-64 receipts add `baseFee`, denominated in the fee currency; `feeCurrency` is a transaction field, not a receipt one.
