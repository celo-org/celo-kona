...
pub(crate) struct RetryConfig {
    ...
    /// Random jitter factor (e.g., 0.1 for ±10% jitter)
    pub jitter: f64,
}
...
impl RetryConfig {
    ...
    pub(crate) fn new(
        ...
        Self { initial_timeout, max_retries, backoff_multiplier, max_timeout, jitter }
    }
    ...
    fn timeout_for_attempt(&self, attempt: u32) -> Duration {
        ...
        let jitter_factor = if self.jitter > 0.0 {
            ...
            // Generate random value between (1.0 - jitter) and (1.0 + jitter)
            rng.gen_range((1.0 - self.jitter)..=(1.0 + self.jitter))
        } else {
            1.0
        };

        let jittered_timeout_secs = timeout_secs * jitter_factor;
        let calculated_timeout = Duration::from_secs_f64(jittered_timeout_secs);
