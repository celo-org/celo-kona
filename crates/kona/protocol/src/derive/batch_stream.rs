                ...
                Batch::Span(b) => {
                    let (validity, _) = b
                        .check_batch_prefix(
                            ...
                            &mut L2BlockInfoFetcher { inner: &mut self.fetcher },
                        )
                        .await;
...
struct L2BlockInfoFetcher<'a, BF>
where
    BF: CeloBatchValidationProvider + Send + Debug,
{
    inner: &'a mut BF,
}
    ...
    BF: CeloBatchValidationProvider + Send + Debug,
{
    ...
    async fn l2_block_info_by_number(&mut self, number: u64) -> Result<L2BlockInfo, Self::Error> {
        self.inner.l2_block_info_by_number(number).await
    }
