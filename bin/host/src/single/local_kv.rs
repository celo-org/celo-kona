...
impl CeloSingleChainLocalInputs {
    ...
    pub fn new(inner: SingleChainLocalInputs, rollup_config: Option<CeloRollupConfig>) -> Self {
        let rollup_config_json =
            rollup_config.as_ref().and_then(|cfg| serde_json::to_vec(cfg).ok()).map(Arc::new);
...
mod tests {
    ...
    /// fields intact — the behaviour the upstream store lacks (it serializes a bare
    /// `RollupConfig`).
    #[test]
    fn serves_celo_rollup_config_with_espresso_fields() {
        ...
        let decoded: CeloRollupConfig = serde_json::from_slice(&bytes)
            .expect("served bytes must deserialize as CeloRollupConfig");
