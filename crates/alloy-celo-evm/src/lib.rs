...
mod tests {
    ...
    /// populate a field. Because `debug_trace*` replays each tx in a fresh EVM, the rate the
    /// handler validates against is whatever `transact_raw` seeds: a block-start rate that lifts
    /// `base_fee_in_erc20` above the tx's max fee must reject the tx via the base-fee check
    /// (`GasPriceLessThanBasefee`), while a rate that keeps it below must pass validation — and
    /// only then fail downstream, at the ERC-20 debit the empty DB cannot service. A regression
    /// that seeded the field but validated against DB state would still pass the sibling
    /// [`test_transact_raw_seeds_context_from_cache`] (which only reads the field back), yet fail
    /// here.
    #[test]
