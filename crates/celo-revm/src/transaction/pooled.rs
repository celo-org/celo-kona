...
///
/// The difference between this and the [`CeloTxEnvelope`] is that this type does not have the deposit
...
pub enum CeloPooledTransaction {
    ...
    /// A [`TxCip64`] transaction tagged with type 123.
...
impl CeloPooledTransaction {
    ...
    ///
    /// Returns the envelope as error if it is a variant unsupported on ethereum: [`TxCip64`]
    pub fn try_into_eth_envelope(self) -> Result<TxEnvelope, CeloTxEnvelope> {
        match self {
            Self::Legacy(tx) => Ok(tx.into()),
            Self::Eip2930(tx) => Ok(tx.into()),
            Self::Eip1559(tx) => Ok(tx.into()),
            Self::Eip7702(tx) => Ok(tx.into()),
            tx @ Self::Cip64(_) => Err(tx.into_celo_envelope()),
        }
    }

    /// Converts the transaction into the celo [`CeloTxEnvelope`].
    pub fn into_celo_envelope(self) -> CeloTxEnvelope {
        match self {
            Self::Legacy(tx) => tx.into(),
            Self::Eip2930(tx) => tx.into(),
            Self::Eip1559(tx) => tx.into(),
            Self::Eip7702(tx) => tx.into(),
            Self::Cip64(tx) => tx.into(),
        }
    }
    ...
    pub const fn as_cip64(&self) -> Option<&TxCip64> {
        match self {
            Self::Cip64(tx) => Some(tx.tx()),
            _ => None,
        }
    }
}
...
impl From<Signed<TxEip7702>> for CeloPooledTransaction {
...
impl TryFrom<CeloPooledTransaction> for alloy_consensus::transaction::PooledTransaction {
    type Error = CeloPooledTransaction;

    fn try_from(value: CeloPooledTransaction) -> Result<Self, Self::Error> {
        match value {
            CeloPooledTransaction::Legacy(tx) => Ok(tx.into()),
            CeloPooledTransaction::Eip2930(tx) => Ok(tx.into()),
            CeloPooledTransaction::Eip1559(tx) => Ok(tx.into()),
            CeloPooledTransaction::Eip7702(tx) => Ok(tx.into()),
            tx @ CeloPooledTransaction::Cip64(_) => Err(tx),
        }
    }
}

impl Hash for CeloPooledTransaction {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.trie_hash().hash(state);
    }
}

impl Encodable for CeloPooledTransaction {
...
impl Decodable for CeloPooledTransaction {
    /// Decodes an enveloped [`CeloPooledTransaction`].
    ///
...
impl Encodable2718 for CeloPooledTransaction {
    ...
    fn encode_2718_len(&self) -> usize {
        match self {
            ...
            Self::Cip64(tx) => tx.eip2718_encoded_length(),
        }
    }
    ...
    fn encode_2718(&self, out: &mut dyn alloy_rlp::BufMut) {
        match self {
            ...
            Self::Cip64(tx) => tx.eip2718_encode(out),
        }
    }
...
impl Decodable2718 for CeloPooledTransaction {
    fn typed_decode(ty: u8, buf: &mut &[u8]) -> Eip2718Result<Self> {
            ...
            .map_err(|_| alloy_rlp::Error::Custom("unexpected tx type"))?
        {
            CeloTxType::NonCeloTx(OpTxType::Eip2930) => {
                Ok(TxEip2930::rlp_decode_signed(buf)?.into())
            }
            CeloTxType::NonCeloTx(OpTxType::Eip1559) => {
                Ok(TxEip1559::rlp_decode_signed(buf)?.into())
            }
            CeloTxType::NonCeloTx(OpTxType::Eip7702) => {
                Ok(TxEip7702::rlp_decode_signed(buf)?.into())
            }
            CeloTxType::Cip64 => Ok(TxCip64::rlp_decode_signed(buf)?.into()),
            CeloTxType::NonCeloTx(OpTxType::Legacy) => {
                Err(Eip2718Error::UnexpectedType(OpTxType::Legacy.into()))
            }
            CeloTxType::NonCeloTx(OpTxType::Deposit) => {
                Err(Eip2718Error::UnexpectedType(OpTxType::Deposit.into()))
            }
        }
    }
...
impl Transaction for CeloPooledTransaction {
...
impl Typed2718 for CeloPooledTransaction {
    fn ty(&self) -> u8 {
        match self {
...
impl IsTyped2718 for CeloPooledTransaction {
    fn is_type(type_id: u8) -> bool {
        // legacy | eip2930 | eip1559 | eip7702 | cip64
        matches!(type_id, 0 | 1 | 2 | 4 | 123)
    }
}

impl TryFrom<CeloPooledTransaction> for TxEnvelope {
    type Error = CeloTxEnvelope;

    fn try_from(tx: CeloPooledTransaction) -> Result<Self, Self::Error> {
        tx.try_into_eth_envelope()
    }
}

impl From<CeloPooledTransaction> for CeloTxEnvelope {
    fn from(tx: CeloPooledTransaction) -> Self {
        tx.into_celo_envelope()
    }
}
...
mod tests {
    ...
    fn invalid_legacy_pooled_decoding_input_too_short() {
        ...
        for hex_data in &input_too_short {
            ...
            let res = CeloPooledTransaction::decode(input_rlp);
            ...
            let input_rlp = &mut &hex_data[..];
            let res = CeloPooledTransaction::decode_2718(input_rlp);
    ...
    fn decode_eip1559_enveloped() {
        ...
        let res = CeloPooledTransaction::decode_2718(&mut &data[..]).unwrap();
        assert_eq!(
            res.to(),
    ...
    fn legacy_valid_pooled_decoding() {
        ...
        let res = CeloPooledTransaction::decode_2718(&mut &data[..]);
        assert!(res.is_ok());
    }
}
