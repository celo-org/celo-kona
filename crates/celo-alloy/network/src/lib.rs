...
impl NetworkTransactionBuilder<Celo> for CeloTransactionRequest {
    ...
    fn can_submit(&self) -> bool {
        ...
        // and gas fields of an `eth_sendTransaction`. Conflicts are another matter, since
        // `prep_for_submission` trims through the Ethereum impl, which cannot see the fee
        // currency. Report them here rather than dropping fees the caller set.
